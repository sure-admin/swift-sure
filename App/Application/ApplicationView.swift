...
struct ApplicationView: View {
  ...
  init(configureNotifications: (NotificationManager, BackendAccessGate) -> Void = { _, _ in }) {
    ...
    makeAssistantServices = {
      ...
      let mcpClient = SureMCPClient(session: services.session, dataTransport: services.dataTransport, makeID: { UUID() })
      let service = LocalAssistantService(financeData: finance.financeData, mcpService: {
      ...
      #else
      return AssistantSessionServices(localAssistant: LocalAssistantService(financeData: finance.financeData))
      #endif
    }
